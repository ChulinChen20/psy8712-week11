...
# Analysis
split <- sample(1:nrow(gss_tbl), .75*nrow(gss_tbl))

gss_train <- gss_tbl[split, ]

gss_test <- gss_tbl[-split, ]
...
write_csv(data.frame(table1_tbl),"../out/table1.csv")
write_csv(data.frame(table2_tbl),"../out/table2.csv")
...
# Q2: (xgbTree)134.194 - (glmnet)7.248 = 126.946.The difference is proportional to
...
# that subtrees in xgbTree takes longer to run compared to individual glmnet models.
# Q3: elastic net: because it provides among the best prediction on both training 
